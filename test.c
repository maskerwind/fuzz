...
int test4(FILE* printFile) {
	...
	ASSERT(sc->root->left == NULL, "left child of noot' node is not NULL");
...
int test5(FILE* printFile) {
    ...
    data* noot = data_new(5, "noot");
    data* mies = data_new(15, "mies");
    data* wim = data_new(12, "wim");
    data* zus = data_new(20, "zus");
    ...
    ASSERT(!data_compare(wim, sc->root->right->left->data), "data is not deleted");
    ASSERT(wim == sc->root->right->left->data, "data is not deleted");

    ASSERT(!data_compare(zus, sc->root->right->right->data), "data is not deleted");
    ASSERT(zus == sc->root->right->right->data, "data is not deleted");

    sortedcontainer_erase(sc, mies);

    ASSERT(!data_compare(aap, sc->root->data), "data is not equivalent");
    ASSERT(aap == sc->root->data, "data is not the same instant");

    ASSERT(!data_compare(noot, sc->root->left->data), "data is not deleted");
    ASSERT(noot == sc->root->left->data, "data is not deleted");

    ASSERT(!data_compare(wim, sc->root->right->data), "data is not deleted");
    ASSERT(wim == sc->root->right->data, "data is not deleted");

    ASSERT(!data_compare(zus, sc->root->right->right->data), "data is not deleted");
    ASSERT(zus == sc->root->right->right->data, "data is not deleted");

    ASSERT(sc->root->right->left == NULL, "left child of wim' node is not NULL");
    ASSERT(sc->root->right->left == NULL, "right child of wim' node is not NULL");
